...
describe('using secondary stream', () => {
    test('stops buffering when the consumer is done', async () => {
        ...
        const lines = [];
        for await (const chunk of parse(decodeText(streamToAsyncIterator(fileBufferStream)))) {
            lines.push(chunk, await lastValueFrom(parse(decodeText(streamToAsyncIterator(createReader())))));
            if (lines.length === 6) {
                break;
            }
        }
    ...
    test('throws if a new stream is created after abortion', async () => {
        ...
        controller.abort();
        await expect(() => lastValueFrom(parse(decodeText(streamToAsyncIterator(createReader()))))).rejects.toThrowErrorMatchingInlineSnapshot('[Error: Cannot create new buffered readers on aborted stream]');
    });
});
describe('cleanup', () => {
...
//# sourceMappingURL=bufferThroughFile.test.js.map
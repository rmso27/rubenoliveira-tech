...
export class RunMigrationCommand extends SanityCommand {
    ...
    async run() {
        ...
        if (!project && !projectId) {
            this.error('sanity.cli.js does not contain a project identifier ("api.projectId") and no --project option was provided.', { exit: 1 });
        }
        if (!dataset && !datasetFromConfig) {
            this.error('sanity.cli.js does not contain a dataset identifier ("api.dataset") and no --dataset option was provided.', { exit: 1 });
        }
        if (!id) {

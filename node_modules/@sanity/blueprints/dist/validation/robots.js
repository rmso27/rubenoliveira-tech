 ...
 */
export function validateRobot(resource) {
    if (!resource)
        return [{ type: 'invalid_value', message: 'Robot config must be provided' }];
    if (typeof resource !== 'object')
        return [{ type: 'invalid_type', message: 'Robot config must be an object' }];
...
export function validateRobotMembership(membership) {
    if (!membership)
        return [{ type: 'invalid_value', message: 'Membership config must be provided' }];
    if (typeof membership !== 'object')
        return [{ type: 'invalid_type', message: 'Membership config must be an object' }];
    const errors = [];
